...
/// Profile metadata: title, description and icon shown by the tray menu and the
/// flyout, and the identifier sent to the elevated helper over the named pipe (it
/// must exactly match the values accepted by BatteryChargeHelper.ps1) - the same
/// identifier is also what gets saved in state.json.
...
{
    // The order here determines the order of the items in the menu and the flyout.
